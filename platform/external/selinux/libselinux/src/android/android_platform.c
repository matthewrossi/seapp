...
static void free_pp(char** pointer)
{
    if (pointer) {
        size_t size = sizeof(pointer)/sizeof(pointer[0]);
...
static char **get_pkg_contexts(const char *filename){
    FTS *fts;
    FTSENT *ftsent;
    char path[] = "/data/selinux";
    char *const paths[] = { path , NULL };
    int ftsflags = FTS_NOCHDIR | FTS_PHYSICAL;
    int sverrno;
    int no_pkgs = 0, cur_pkg = 0;
    char **contexts_pkg = NULL, **ret;
    ...
    while ((ftsent = fts_read(fts)) != NULL) {
        ...
        case FTS_F:
            if (!contexts_pkg) {
                contexts_pkg = (char **) calloc(no_pkgs, sizeof(char *));
                if (!contexts_pkg) {
            ...
            if (!strcmp(ftsent->fts_name, filename) && cur_pkg < no_pkgs) {
                contexts_pkg[cur_pkg] = malloc((ftsent->fts_pathlen + 1) * sizeof(char));
                strncpy(contexts_pkg[cur_pkg], ftsent->fts_path, ftsent->fts_pathlen + 1);
                cur_pkg++;
            }
        }
    }
                                                    ...
                                                    unsigned nopts)
{
    ...
    // Compute fc_digest only on the system file_contexts
...
static void free_seapp_contexts(struct seapp_context **seapp_contexts)
{
	int n/*, nspec*/;
...
static struct seapp_context** selinux_android_seapp_context(const char *const *seapp_contexts_files,
                                                            size_t files_len)
{
	FILE *fp = NULL;
    ...
    size_t i, len;
    struct seapp_context **seapp_contexts;
    //int nspec;

    for (i = 0; i < files_len; ++i)
        selinux_log(SELINUX_INFO, "seapp_contexts_files[i] = %s", seapp_contexts_files[i]);
	...
	for (i = 0; i < files_len; i++) {
		...
		if (!fp) {
			selinux_log(SELINUX_ERROR, "%s:  could not open seapp_contexts file: %s",
				    __FUNCTION__, seapp_contexts_files[i]);
			free_seapp_contexts(seapp_contexts);
			return NULL;
		}
		while (fgets(line_buf, sizeof line_buf - 1, fp)) {
			len = strlen(line_buf);
	...
	seapp_contexts_dup = false;

	qsort(seapp_contexts, nspec, sizeof(struct seapp_context *), seapp_context_cmp);
    ...
    selinux_log(SELINUX_INFO, "SELinux: Loaded seapp_contexts\n");
...
int selinux_android_seapp_context_reload(void)
{
    ...
    const char *const *def_seapp_contexts_files;

    // Prefer files from /system & /vendor, fall back to files from /
    if (access(seapp_contexts_split[0], R_OK) != -1) {
        def_seapp_contexts_files = seapp_contexts_split;
        files_len = sizeof(seapp_contexts_split)/sizeof(seapp_contexts_split[0]);
    } else {
        def_seapp_contexts_files = seapp_contexts_rootfs;
        files_len = sizeof(seapp_contexts_rootfs)/sizeof(seapp_contexts_rootfs[0]);
    }

    char **seapp_contexts_files, **pkg_seapp_contexts = get_pkg_contexts("seapp_contexts");
    if (pkg_seapp_contexts) {
        size_t nof_pkgs = sizeof(pkg_seapp_contexts)/sizeof(pkg_seapp_contexts[0]);
        seapp_contexts_files = (char **) calloc(files_len + nof_pkgs, sizeof(char *));
        if (!seapp_contexts_files) {
            selinux_log(SELINUX_ERROR, "%s:  Out of memory\n", __FUNCTION__);
            free_pp(pkg_seapp_contexts);
            return -1;
        }
        for (i = 0; i < files_len; ++i)
            seapp_contexts_files[i] = strdup(def_seapp_contexts_files[i]);
        for (i = 0; i < nof_pkgs; ++i)
            seapp_contexts_files[files_len++] = pkg_seapp_contexts[i];
    } else {
        seapp_contexts_files = (char **) calloc(files_len, sizeof(char *));
        if (!seapp_contexts_files) {
            selinux_log(SELINUX_ERROR, "%s:  Out of memory\n", __FUNCTION__);
            return -1;
        }
    ...
    seapp_contexts = selinux_android_seapp_context(seapp_contexts_files, files_len);
    selinux_log(SELINUX_INFO, "%s:  seapp_contexts = %p\n", __FUNCTION__, seapp_contexts);
    free_pp(seapp_contexts_files);  // modify free_pp and get_pkg_contexts to avoid getting always lenght = to 1
    return 0;
}
...
static int seapp_context_lookup(/*seapp_context **seapp_contexts,*/
                enum seapp_kind kind,
				uid_t uid,
				bool isSystemServer,
				const char *seinfo,
				const char *pkgname,
				...
				context_t ctx)
{
	...
	//size_t i, nspec = sizeof(seapp_contexts)/sizeof(seapp_contexts[0]);
	int i;
	uid_t userid;
	uid_t appid;
	...
	selinux_log(SELINUX_INFO, "SELinux: Lookup into seapp_contexts\n");
			       ...
			       const char *pkgname)
{
    ...
    selinux_log(SELINUX_INFO, "%s:  Executing security_check_context ctx=%s\n", __FUNCTION__, ctx_str);
	rc = security_check_context(ctx_str);  // ERROR: invalid context SOLVED: adding (typeattributeset domain com_example_sepolicytestapp.init) or (roletype r com_example_sepolicytestapp.init)
	if (rc < 0)
		goto err;

    selinux_log(SELINUX_INFO, "%s:  Executing strcmp ctx=%s orig_ctx=%s\n", __FUNCTION__, ctx_str, orig_ctx_str);
	if (strcmp(ctx_str, orig_ctx_str)) {
		rc = selinux_android_setcon(ctx_str);   // ERROR: Permission denied SOLVED: adding (typeattributeset appdomain com_example_sepolicytestapp.init)
		if (rc < 0)
			goto err;
	}
...
/* The path prefix of package sepolicy directories. */
#define DATA_SELINUX_PREFIX "/data/selinux/"

/* The suffix of package file_contexts file. */
#define FC_SUFFIX "/file_contexts"
                                 ...
                                 char **secontextp)
{
    ...
    *end = '\0';
    ...
    char pkg_sepolicy_dir[strlen(DATA_SELINUX_PREFIX) + strlen(pkgname) + 1];
    strcpy(pkg_sepolicy_dir, DATA_SELINUX_PREFIX);
    strcat(pkg_sepolicy_dir, pkgname);

    ctx = context_new(secontext);
    if (!ctx)
        goto err;

    if (access(pkg_sepolicy_dir, R_OK) == -1) {
        ...
        selinux_log(SELINUX_INFO, "%s:  app with uid %d, seinfo %s, name %s is classic\n",
            __FUNCTION__, info ? info->uid : uid, info ? info->seinfo : seinfo, pkgname);
        rc = seapp_context_lookup(SEAPP_TYPE, info ? info->uid : uid, 0,
                                      info ? info->seinfo : seinfo, pkgname, pathname, ctx);
        if (rc < 0)
            goto err;
        ...
        // app has its own policy and contexts
        selinux_log(SELINUX_INFO, "%s:  app with name %s has its own policy\n", __FUNCTION__, pkgname);   // on installation prints twice!!!

        // store pkg_fc_sehandle doesn't improve performance since an hash of file_contexts should
        // be computed anyway
        ...
        selinux_log(SELINUX_INFO, "SELinux: Loaded %s's file_contexts\n", pkgname);
        ...
        if (selabel_lookup(pkg_fc_sehandle, &secontext, pathname, sb.st_mode) < 0) {   // will lookup only from the pkgdir as it is, if *empty* should be /
            rc = 0; // no match, but not an error
            goto out;
        }
    }

    selinux_log(SELINUX_INFO, "SELinux: path %s, pkgname %s\n", pathname, pkgname);
...
out:
    ...
    return rc;
err:
    selinux_log(SELINUX_ERROR, "%s:  Error looking up context for path %s, pkgname %s: %s\n",
                __FUNCTION__, pathname, pkgname, strerror(errno));
    rc = -1;
    goto out;
}
                         ...
                         const char *seinfo, uid_t uid)
{
    ...
    selinux_log(SELINUX_INFO, "SELinux: Lookup into file_contexts\n");
                                             ...
                                             unsigned int flags)
{
    ...
    bool recurse = (flags & SELINUX_ANDROID_RESTORECON_RECURSE) ? true : false;     // depending on restorecon_app_data_lazy
    bool force = (flags & SELINUX_ANDROID_RESTORECON_FORCE) ? true : false;
    bool datadata = (flags & SELINUX_ANDROID_RESTORECON_DATADATA) ? true : false;   // true
