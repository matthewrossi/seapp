...
static void SpecializeCommon(JNIEnv* env, uid_t uid, gid_t gid, jintArray gids,
  ...
  const char* app_data_dir_ptr = app_data_dir.has_value() ? app_data_dir.value().c_str() : nullptr;

  const char* pkgname = nullptr;
  if (app_data_dir_ptr != nullptr) {
    pkgname = strrchr(app_data_dir_ptr, '/') + 1;
  }

  if (selinux_android_setcontext(uid, is_system_server, se_info_ptr, nice_name_ptr, pkgname) == -1) {
    fail_fn(CREATE_ERROR("selinux_android_setcontext(%d, %d, \"%s\", \"%s\", \"%s\") failed", uid,
          is_system_server, se_info_ptr, nice_name_ptr, pkgname));
  }
