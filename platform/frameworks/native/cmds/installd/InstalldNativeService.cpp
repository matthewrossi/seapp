...
std::vector<std::string> get3rdParty(const char *filename) {
    ...
    while ((ent = readdir(dir))) {
        if (strcmp(ent->d_name, ".") && strcmp(ent->d_name, "..")) {
            std::string pkg_path(path + ent->d_name + "/");
            DIR *pkg_dir = opendir(pkg_path.c_str());
            if (pkg_dir == NULL) {
                return {};
            }
            while ((ent = readdir(pkg_dir))) {
                if (!strcmp(ent->d_name, filename)) {
                    policies.push_back(pkg_path + ent->d_name);
                }
            }
            closedir(pkg_dir);
        }
...
binder::Status InstalldNativeService::reloadSELinuxPolicy() {
    ...
    char compiled_sepolicy[] = "/data/cache/selinux/sepolicy.XXXXXX";
    ...
    if (access(vendor_policy_cil_file.c_str(), F_OK) == -1) {
        ...
        return error(StringPrintf("Missing %s", plat_pub_versioned_cil_file.c_str()));
    }
