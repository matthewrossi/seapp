...
// TODO: move policy reload code in external/selinux/libselinux/src/android/android_platform.c
//       so that it could be called from installd too
...
static Result<Success> do_load_policy(const BuiltinArguments& args) {
    ...
    if (compiled_sepolicy_fd < 0) {
        return Error() << "Failed to create temporary file " << compiled_sepolicy;
    }
    ...
    if (access(vendor_policy_cil_file.c_str(), F_OK) == -1) {
        ...
        return Error() << "Missing " << plat_pub_versioned_cil_file.c_str();
    }
    ...
    if (selinux_android_load_policy_from_fd(compiled_sepolicy_fd, compiled_sepolicy) < 0) {
        return Error() << "Failed to load SELinux policy from " << compiled_sepolicy;
    }
...
const BuiltinFunctionMap::Map& BuiltinFunctionMap::map() const {
    ...
    static const Map builtin_functions = {
        ...
        {"load_policy",             {0,     0,    {false,   do_load_policy}}},
