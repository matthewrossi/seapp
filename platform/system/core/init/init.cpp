...
int main(int argc, char** argv) {
    ...
    if (is_first_stage) {
        ...
        /*
            [    4.202891] init: init first stage started!
            [    4.203108] init: Using Android DT directory /proc/device-tree/firmware/android/
            [    4.203159] init: First stage mount skipped (missing/incompatible fstab in device tree)

            /proc/device-tree/firmware/android/fstab/compatible doesn't exist, so can't be read

            fstab.aosp_angler   remove system and vendor partition
            fstab.angler         find out if needs to change too (TARGET_RECOVERY_FSTAB)

        */

        SetInitAvbVersionInRecovery();

        LOG(INFO) << "Split policy device? " << selinux_is_split_policy_device();
