...
/*
 * Function: setCon
 * Purpose:  set the security context of the current process
 * Parameters: context: the new security context of the current process
 * Returns: true on success, false on error
 * Exceptions: none
 */
static jboolean setCon(JNIEnv *env, jobject, jstring contextStr) {
    ...
    if (context.c_str() == NULL) {
        ALOGV("%s(%p): NULL pointer exception",__FUNCTION__ , contextStr);
        return false;
    }

    // GetStringUTFChars returns const char * yet setfilecon needs char *
    char *orig_ctx_str = NULL, *ctx_str = const_cast<char *>(context.c_str());
    int rc = -1;

    rc = getcon(&orig_ctx_str);
    if (rc)
        goto err;
    ALOGD("Current context: %s New context: %s.", orig_ctx_str, ctx_str);

    ALOGV("%s(%s):  Executing security_check_context ctx=%s", __FUNCTION__, ctx_str, ctx_str);
    ...
    if (rc < 0)
        goto err;
    ALOGD("Security check for context %s passed.", ctx_str);

    ALOGV("%s(%s):  Executing strcmp ctx=%s orig_ctx=%s", __FUNCTION__, ctx_str, ctx_str, orig_ctx_str);
    if (strcmp(ctx_str, orig_ctx_str)) {
        ALOGD("Going to execute selinux_android_setcon.");
...
out:
    ...
    ALOGV("%s(%s) => %s", __FUNCTION__, ctx_str, context.c_str());
    return (rc == 0);
err:
    ALOGV("%s(%s): Error setting context %s", __FUNCTION__, ctx_str, strerror(errno));
    rc = -1;
    goto out;
}
...
static const JNINativeMethod method_table[] = {
    ...
    { "setContext"               , "(Ljava/lang/String;)Z"                        , (void*)setCon           },
