...
static pid_t ForkAndSpecializeCommon(JNIEnv* env, uid_t uid, gid_t gid, jintArray javaGids,
  ...
  if (pid == 0) {
    ...
    const char* data_dir_c_str = NULL;
    ScopedUtfChars* data_dir = NULL;
    if (dataDir != NULL) {
        data_dir = new ScopedUtfChars(env, dataDir);
        data_dir_c_str = data_dir->c_str();
        if (data_dir_c_str == NULL) {
          fail_fn("data_dir_c_str == NULL");
        } else {
          pkgname = strrchr(data_dir_c_str, '/') + 1;
        }
    }
    rc = selinux_android_setcontext(uid, is_system_server, se_info_c_str, se_name_c_str, pkgname);
    if (rc == -1) {
      fail_fn(CREATE_ERROR("selinux_android_setcontext(%d, %d, \"%s\", \"%s\", \"%s\") failed", uid,
            is_system_server, se_info_c_str, se_name_c_str, pkgname));
    }
