...
static int restorecon_app_data_lazy(const std::string& path, const std::string& seInfo, uid_t uid,
    ...
    if (selinux_android_restorecon_pkgdir(path.c_str(), seInfo.c_str(), uid, SELINUX_ANDROID_RESTORECON_VERBOSE/*0*/) < 0) {
    ...
    if (strcmp(before, after)) {
        ...
        if (selinux_android_restorecon_pkgdir(path.c_str(), seInfo.c_str(), uid,
                SELINUX_ANDROID_RESTORECON_VERBOSE | SELINUX_ANDROID_RESTORECON_RECURSE) < 0) {
...
// TODO: try to move all policy reload code in android_platform.c of external/libselinux
//       so that it could potentially be called also from init
...
static char **get_pkg_policy(const char *filename) {
    FTS *fts;
    FTSENT *ftsent;
    ...
    while ((ftsent = fts_read(fts)) != NULL) {
        ...
        case FTS_DC:
            PLOG(ERROR) << "SELinux:  Directory cycle on "
                        << ftsent->fts_path << ".\n";
            errno = ELOOP;
            ret = NULL;
            goto err;
        case FTS_DNR:
            PLOG(ERROR) << "SELinux:  Could not read "
                        << ftsent->fts_path << ": " << strerror(errno) << ".\n";
            fts_set(fts, ftsent, FTS_SKIP);
            continue;
        case FTS_NS:
            PLOG(ERROR) << "SELinux:  Could not stat "
                        << ftsent->fts_path << ": " << strerror(errno) << ".\n";
            fts_set(fts, ftsent, FTS_SKIP);
            continue;
        case FTS_ERR:
            PLOG(ERROR) << "SELinux:  Error on "
                        << ftsent->fts_path << ": " << strerror(errno) << ".\n";
            fts_set(fts, ftsent, FTS_SKIP);
            continue;
        ...
        case FTS_F:
            if (!contexts_pkg) {
                ...
                if (!contexts_pkg) {
                    PLOG(ERROR) << __FUNCTION__ << ":  Out of memory\n";
                    ret = NULL;
                    goto out;
                }
            }
            if (!strcmp(ftsent->fts_name, filename) && cur_pkg < no_pkgs) {
                contexts_pkg[cur_pkg] = (char *) malloc((ftsent->fts_pathlen + 1) * sizeof(char));
...
static int selinux_android_load_policy_from_fd(int fd, const char *description)
{
	int rc;
	struct stat sb;
	void *map = NULL;
 ...
 */
static bool fork_execv_and_wait_for_completion(const char* filename, char* const argv[]) {
...
binder::Status InstalldNativeService::reloadSELinuxPolicy() {
    ...
    if (max_policy_version == -1) {
        return error("Failed to determine highest policy version supported by kernel");
    }

    // TODO: fix cache/selinux context and its policy files context
    char compiled_sepolicy[] = "/cache/selinux/sepolicy.XXXXXX";
    android::base::unique_fd compiled_sepolicy_fd(mkostemp(compiled_sepolicy, O_CLOEXEC));
    if (compiled_sepolicy_fd < 0) {
        return error(StringPrintf("Failed to create temporary file %s", compiled_sepolicy));
    }

    // Determine which mapping file to include
    std::string vend_plat_vers;
    if (!selinux_get_vendor_mapping_version(&vend_plat_vers)) {
        return error("Failed to determine vendor mapping version");
    }
    ...
    char **compile_args, **pkg_policies = get_pkg_policy("sepolicy.cil");
    if (pkg_policies) {
        size_t nof_pkgs = sizeof(pkg_policies)/sizeof(pkg_policies[0]);
        compile_args = (char **) calloc(len + nof_pkgs + 1, sizeof(char *));
        if (!compile_args) {
    ...
    /*
        Failed to resolve typeattributeset statement at /system/etc/selinux/mapping/27.0.cil:967
        adb shell sed -n 967p /system/etc/selinux/mapping/27.0.cil
        (typeattributeset cache_selinux_file_27_0 (cache_selinux_file))
        Need to modify the nonplat_sepolicy.cil on the vendor.img
    */

    if (!fork_execv_and_wait_for_completion(compile_args[0], (char**)compile_args)) {
    ...
    if (selinux_android_load_policy_from_fd(compiled_sepolicy_fd, compiled_sepolicy) < 0) {
        return error(StringPrintf("Failed to load SELinux policy from %s", compiled_sepolicy));
    }
